...
import java.io.IOException; // Imports library for I/O
...
void setup() {
  ...
  smooth();
  myPort = new Serial(this, "COM3", 115200); // Serial communication (Write Arduino COM port here!)
  myPort.bufferUntil('.'); // Get data up to '.' character
}
...
void draw() {
  ...
  noStroke();
  fill(0, 4); 
  rect(0, 0, width, height - height*0.065); // Fades the radar screen
  fill(98, 245, 31); 
  drawRadar();       // Radar draws
  drawLine();        // moving line
  drawObject();      // draws object
  drawText();        // Writes texts to the screen
}
...
void drawObject() {
  ...
  if (iDistance > 10 && iDistance < 40) {
     line(pixsDistance*cos(radians(iAngle)), -pixsDistance*sin(radians(iAngle)));
        (width - width*0.0505)*cos(radians(iAngle)), -(width - width*0.505)*sin(radians(iAngle)));
  }
  popMatrix();
}
...
void drawText() {
  ...
  if (iDistance >= 10 && iDistance <= 40) {
   text(iDistance + " cm", width - width * 0.14, height - height * 0.0277);
    ...
    text("  ", width - width * 0.14, height - height * 0.0277);
  }
